...
3.  查看文件编码，git状态，文件格式和当前状态灯信息(Powerline)
    *  :set ft=html
        <blockquote>将当前打开的文件类型设置为html,同理可设为javascript，css等</blockquote>
    *  通过安装powerline插件来方便的显示当前模式，文件状态，git状态，文件类型等信息
    *   跟多信息：[link](https://github.com/Lokaltog/powerline)

4.  zencoding(节省编写html页面的时间)
...
6.  缩进提示(nathanaelkane/vim-indent-guides)
